...
3. After a successful login, `AppGui` loads the chat history and updates the view. `SidebarPanel` is updated with the current online user list.
...
5. Every incoming server message is handed to the Tkinter main thread via `root.after` for a  GUI update. Depending on the message type - a chat message, a user list update, or a system notification - it is routed to the correct part of the interface.
...
![Chat Application Flowchart](chat_flowchart.png)

---

## Module Structure

![Chat Application File Structure](chat_filestructure.png)
